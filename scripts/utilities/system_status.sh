...
show_database_statuses() {
    ...
    # Dynamically read databases from YAML configuration
    local db_helper="$(dirname "${BASH_SOURCE[0]}")/helpers/read_eurostyle_dbs.py"
    
    if [[ ! -x "$db_helper" ]]; then
        log_error "Database configuration helper not found: $db_helper"
        return 1
    fi
    
    # Load database entries from configuration
    mapfile -t DB_ENTRIES < <(python3 "$db_helper" 2>/dev/null)
    
    if [[ ${#DB_ENTRIES[@]} -eq 0 ]]; then
        log_error "No EuroStyle databases found in configuration"
        return 1
    fi
    ...
    # Process each database entry from configuration
    ...
    log_info "Databases loaded from configuration: ${#DB_ENTRIES[@]} found"
